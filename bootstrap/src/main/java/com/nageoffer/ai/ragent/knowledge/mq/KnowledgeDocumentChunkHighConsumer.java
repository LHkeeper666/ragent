...
@RocketMQMessageListener(
        topic = "knowledge-document-chunk-high_topic${unique-name:}",
        consumerGroup = "knowledge-document-chunk-high_cg${unique-name:}",
        consumeThreadNumber = 8
)
public class KnowledgeDocumentChunkHighConsumer implements RocketMQListener<MessageWrapper<KnowledgeDocumentChunkEvent>> {
    ...
    public void onMessage(MessageWrapper<KnowledgeDocumentChunkEvent> message) {
        ...
        log.info("[consumer] start high priority document chunk task, docId={}, keys={}", event.getDocId(), message.getKeys());
