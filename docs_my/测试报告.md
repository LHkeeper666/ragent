...
本次迭代新增 high / medium / low 三级动态任务队列，覆盖知识库文档解析和 `ingestion/tasks` 两条异步链路。测试重点从“检索质量”扩展到“任务调度正确性”和“排队状态可见性”，验证系统能在小文件、大文件、批量积压同时存在时保持资源隔离。

### 11.2 测试范围

| 模块 | 测试点 | 测试类型 |
|------|--------|---------|
| `TaskQueueService` | 优先级判定、等待时间估算 | 单元测试 |
| 知识库文档解析 | `startChunk` 入队、队列状态写入、topic 选择 | 集成测试 |
| Ingestion 任务 | create/upload 异步入队、消费者执行 Pipeline | 集成测试 |
| 数据库迁移 | 队列字段、索引、文件元数据字段 | DDL 验证 |
| 前端页面 | 文档列表、任务列表、详情弹窗的排队信息展示 | UI 验证 |

### 11.3 单元测试结果

新增 `TaskQueueServiceTest` 覆盖优先级判定边界。执行命令如下：

```bash
.\mvnw.cmd -pl bootstrap -am -Dtest=TaskQueueServiceTest "-Dsurefire.failIfNoSpecifiedTests=false" -DforkCount=0 test
```

| 用例 | 输入条件 | 期望结果 | 状态 |
|------|----------|----------|------|
| 小文件 + 少量待处理 | 待处理数 ≤ 3，文件 ≤ 1MB | high | 通过 |
| 常规文件 | 未命中 high/low 条件 | medium | 通过 |
| 大文件 | 文件 ≥ 100MB | low | 通过 |
| 大量积压 | 待处理数 ≥ 20 | low | 通过 |
| 低优先级优先命中 | 大文件同时满足其他条件 | low | 通过 |
...
| 场景 | 验证内容 | 预期结果 |
|------|----------|----------|
| 文档解析入队 | 调用 `startChunk(docId)` 后不直接执行解析 | 文档状态进入 queued/pending，记录 priority 和 queued_at |
| 消费者领取 | 高/中/低消费者收到对应 topic 消息 | 开始消费时写入 running 和 queue_started_at |
| 解析完成 | 消费者执行现有分块逻辑 | 成功后保持 success，失败后保持 failed 和 chunk log |
| Ingestion 上传 | 上传文件后接口立即返回 | 文件落对象存储，任务状态为 queued/pending |
| Ingestion 消费 | 消费者读取文件 URL 并调用 `IngestionEngine` | 领取后进入 running，完成后进入 completed/failed |
| 优先级隔离 | 低优任务存在时新增高优小文件 | 高优任务进入高优 topic，不被低优消费者占用 |

### 11.5 前端验证

| 页面 | 验证项 | 预期结果 |
|------|--------|----------|
| 知识文档列表 | 状态列展示 priority / queueStatus / queuePosition | 用户能看到“高/中/低”和排队位置 |
| 文档详情 | 展示文件大小、入队时间、开始时间、预计等待 | 字段存在时正常展示，空值时不破坏布局 |
| 分块日志弹窗 | 展示队列状态与运行状态 | pending/queued/running/success/failed 使用不同 badge |
| Ingestion 任务页 | 优先级筛选与队列状态列 | 可按 high/medium/low 筛选任务 |
| 上传反馈 | 上传完成 toast | 文案显示“已入队”，不再暗示解析已完成 |

### 11.6 构建验证

| 工具 | 命令 | 结果 |
|------|------|------|
| Maven 编译 | `.\mvnw.cmd -pl bootstrap -am -DskipTests compile` | 通过 |
| 前端构建 | `npm.cmd run build` | 通过 |
| 队列单元测试 | `.\mvnw.cmd -pl bootstrap -am -Dtest=TaskQueueServiceTest "-Dsurefire.failIfNoSpecifiedTests=false" -DforkCount=0 test` | 通过 |

前端构建过程中仍存在 Vite chunk 体积提示和 browserslist 数据过期提示，属于原有构建告警，不影响本次队列功能验证。

### 11.7 风险与改进建议

| 风险 | 影响 | 建议 |
|------|------|------|
| 预计等待时间为估算值 | 用户可能将其理解为精确 SLA | 前端文案使用“预计”表达，并结合实际吞吐后调参 |
| RocketMQ topic 增多 | 运维配置复杂度上升 | 将 topic 名称和消费者并发集中配置 |
| 大文件对象存储读取失败 | Ingestion 消费失败 | 保留失败状态和错误日志，后续补充重试策略 |
| 排队位置依赖数据库统计 | 高并发下位置可能短暂变化 | 将其作为展示字段，不作为调度依据 |
