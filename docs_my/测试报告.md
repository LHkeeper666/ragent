...
### 4.5 联网搜索功能（WebSearchService）

#### 4.5.1 实现原理验证

联网搜索基于 `WebSearchService` → `DefaultWebSearchService`，支持 Tavily（主）和 DuckDuckGo（兜底）双引擎：

| 验证项 | 实现方式 | 结果 |
|--------|---------|------|
| Tavily AI 总结 | `include_answer=true`，parseTavilyResponse 优先提取 answer 字段 | 正确，AI总结作为首个结果 |
| Tavily 结构化结果 | 提取 results 数组的 title/url/content 字段 | 正确，标注来源 URL |
| DuckDuckGo 即时答案 | 提取 AbstractText + AbstractURL → WebSearchResult | 正确，免费兜底 |
| DuckDuckGo 相关主题 | 解析 RelatedTopics 数组，分离标题和描述 | 正确 |
| 分级降级 | Tavily 无 Key → DuckDuckGo；Tavily 调用失败 → 捕获异常返回空 | 正确 |
| 触发条件 | `webSearchProperties.enabled && hasLowConfidenceIntents(ctx)` | 正确，仅在 KB 不足时触发 |
| 流式输出 | 联网搜索结果通过 SSE Streaming 输出，与 RAG 回答体验一致 | 正确 |

#### 4.5.2 端到端联网搜索测试

基于 `docs_my/联网检索_测试报告.md` 记录的生产日志：

**测试A：知识库无覆盖 → 联网搜索兜底（"Redis介绍"）**

```
关键词检索统计：Chunk 总数 = 0
向量全局检索：检索到 12 个 Chunk（低相关性）
意图置信度不足，启用联网搜索增强
→ Tavily 返回：Redis定义、数据结构、特性、持久化等
→ 输出标注来源：菜鸟教程、Redis中文网、IBM
→ 末尾提示："以上部分信息来源于联网搜索，仅供参考"
```

**测试B：知识库无覆盖 → 联网搜索兜底（"介绍YOLO"）**

```
问题改写：介绍yolo → YOLO介绍
关键词检索：9/9 collection 成功，Chunk 总数 = 0
向量全局检索：12 个 Chunk（低相关性）
意图置信度不足，启用联网搜索增强
→ Tavily 返回：YOLO原理、版本演进（v1-v5）、优势局限
→ 输出来源：知乎专栏、CSDN博客、PyTorch Book
```

**测试C：知识库有覆盖 → 不触发联网搜索（"PyTorch搭建CNN步骤"）**

```
意图识别：topic_pytorch (score=0.95)
启用通道：[IntentDirectedSearch, KeywordSearch]
关键词检索：6 个 Chunk（7ms）
意图定向检索：5 个 Chunk（886ms）
→ 未触发联网搜索（意图置信度高 + 检索有结果）
→ 正常 RAG 链路完成
```

#### 4.5.3 联网搜索测试汇总

| 测试 | 知识库覆盖 | 触发联网 | Tavily返回 | 效果 |
|------|-----------|---------|-----------|------|
| Redis介绍 | 无 | 是 | AI总结 + 结构化结果 | 完整回答，标注来源 |
| 介绍YOLO | 无 | 是 | 算法原理 + 版本演进 + 来源URL | 专业级回答 |
| PyTorch搭建CNN | 有 | 否 | 不触发 | 正常RAG链路 |

联网搜索触发条件验证：仅在"KB无相关 + 意图置信度低"时触发，KB已有覆盖时不误触发。

#### 4.5.4 降级策略验证

| 场景 | 预期行为 | 验证 |
|------|---------|------|
| Tavily API Key 未配置 | 自动降级到 DuckDuckGo | 代码逻辑验证通过 |
| Tavily HTTP 非200 | 返回空列表，不崩溃 | catch后 `return Collections.emptyList()` |
| Tavily 超时 | 捕获异常，返回空列表 | `timeout(Duration.ofSeconds(15))` + try-catch |
| 联网搜索无结果 + KB无内容 | 返回"未检索到相关内容，联网搜索也未找到相关信息" | callback提示用户 |
| 联网搜索无结果 + KB有内容 | 回退到正常 RAG 链路 | `return false` 回退 |
...
以测试C "PyTorch搭建CNN步骤" Trace 为例，验证完整的评测触发链路：

```
[StreamChatEventHandler] 评测事件已投递: traceId=2063531784519286784
[RagEvalConsumer] 收到评测事件, traceId=2063531784519286784
[RagEvaluationServiceImpl] 评测完成: traceId=2063531784519286784, 指标数=3
```

| 步骤 | 组件 | 验证内容 | 状态 |
|------|------|---------|------|
| 1. 暂存 | `EvalPendingStore` | 问题+上下文在 SSE 流开始时暂存到 ConcurrentHashMap | 通过 |
| 2. 事件组装 | `StreamChatEventHandler` | 读取 PendingData，组装 RagEvalEvent（traceId/question/answer/kbContext/mcpContext） | 通过 |
| 3. MQ 投递 | `RocketMQProducerAdapter` | 发送结果 SEND_OK，消息 Keys 包含 messageId | 通过 |
| 4. MQ 消费 | `RagEvalConsumer` | 消费组 `rag-eval_cg` 接收事件，调用 evaluate() | 通过 |
| 5. 评测执行 | `RagEvaluationServiceImpl` | 根据 metrics 配置依次执行已启用的评测器（3个） | 通过 |
| 6. 结果落库 | `RagEvalResultMapper.insert` | 每个评测器一条记录写入 `t_rag_eval_result` | 通过 |
| 7. Trace 回写 | `updateTraceExtraData` | 评测结果汇总写入 `t_rag_trace_run.extraData` JSON 字段 | 通过 |

#### 6.6.2 三维度评测器验证

| 评测器 | Bean 名称 | 模板路径 | 输入依赖 | 验证状态 |
|--------|----------|---------|---------|---------|
| FaithfulnessEvaluator | faithfulnessEvaluator | `prompt/eval-faithfulness.st` | 问题 + 答案 + 检索上下文 | 通过 |
| AnswerRelevancyEvaluator | answerRelevancyEvaluator | `prompt/eval-answer-relevancy.st` | 问题 + 答案 | 通过 |
| CorrectnessEvaluator | correctnessEvaluator | `prompt/eval-correctness.st` | 问题 + 答案 + 上下文 + 通用知识 | 通过 |

**评测 Prompt 模板验证**（以 Faithfulness 为例）：

| 验证项 | 内容 |
|--------|------|
| 模板格式 | StringTemplate（.st），`{{question}}` / `{{answer}}` / `{{context}}` 占位符 |
| 评分标准 | score=1.0（全部有依据）→ 0.0（完全捏造），5级量表 |
| 输出格式 | 强制 JSON：`{"score": 0.85, "label": "PASS", "reason": "...", "unsupported_claims": [...], "supported_claims": [...]}` |
| Judge 模型 | 默认 qwen-plus，temperature=0.0，thinking=false |

#### 6.6.3 Judge 客户端容错验证

`RagJudgeClient` 的关键容错机制：

| 场景 | 处理 | 返回值 |
|------|------|--------|
| LLM 调用成功 + JSON 合法 | 正常解析 score/label/reason/evidence | 标准 RagEvalMetricResult |
| LLM 返回 ```json 包裹 | 正则去除 markdown 代码块标记后解析 | 正确解析 |
| LLM 返回不可解析文本 | catch JsonParseException | score=0, label=FAIL, reason="Failed to parse..." |
| LLM 调用异常 | catch Exception | score=0, label=FAIL, reason="Judge model call failed..." |
| score 字段缺失/null | `toBigDecimal(null)` | BigDecimal.ZERO |
| 结果落库失败 | catch per result | 单个失败不影响其他评测器结果，日志记录 |

#### 6.6.4 评测看板数据验证

`EvalStatsService` 管理后台看板接口功能验证：

| API | 验证数据维度 | 状态 |
|-----|------------|------|
| `loadOverview("24h")` | 返回三维度平均分 + 低分数量 + 低分占比 | 通过 |
| `loadTrends("faithfulness", "7d", "1h")` | 按小时分桶统计 avgScore/totalCount/failCount | 通过 |
| `loadLowScoreSamples("correctness", 10)` | 返回 FAIL 标签的低分样本（question/answer/reason） | 通过 |

**关键计算逻辑验证**：

| 计算项 | 公式 | 验证 |
|--------|------|------|
| 平均分 | `sum(scores) / count`（BigDecimal 精算，3位小数） | 通过 |
| 低分占比 | `lowScoreCount / totalCount`（HALF_UP 舍入） | 通过 |
| 时间窗口 | switch(24h/7d/30d/90d) → `LocalDateTime.now().minusXxx()` | 通过 |

#### 6.6.5 采样机制验证

| 验证项 | 配置 | 效果 |
|--------|------|------|
| 采样率 | `rag.eval.sample-rate=0.05`（5%） | 全量对话中仅 5% 触发评测 |
| 指标开关 | `rag.eval.metrics.faithfulness/answer-relevancy/correctness` | 可独立开关各评测器 |
| 评测模式 | evalMode="ONLINE_SAMPLE" | 与离线评测数据隔离 |
| 评测开关 | `rag.eval.enabled=false` | 关闭时不投递任何评测事件 |
...
| DefaultWebSearchService | 无测试 | 中：涉及HTTP调用，回归需Mock验证 | 编写集成测试（Mock Tavily/DuckDuckGo API） |
| EvalController | 无测试 | 低 | 编写Controller层集成测试 |
| RagJudgeClient | 无测试 | 中：JSON解析容错逻辑较多 | 编写单元测试（Mock LLMService） |
...
| Tavily API Key 未配置 | 高 | 低 | 自动降级 DuckDuckGo，功能不中断 | 可忽略 |
| 评测 Prompt 模板设计有偏差 | 中 | 中 | 模板外置可独立调优；temperature=0.0 保证一致性 | 中 |
...
| Tavily 外部API服务不可用 | 中 | 中 | 自动降级 DuckDuckGo；两者均不可用时回退正常 RAG | 低 |
| Judge 模型调用失败 | 低 | 低 | `RagJudgeClient` catch 异常返回 FAIL + score=0 | 低 |
| 评测结果表 (`t_rag_eval_result`) 数据膨胀 | 中 | 低 | 采样率控制（默认5%）；定期清理历史数据预案 | 低 |
...
| 噪声引入 | 0 | 0 | — |
| 联网搜索兜底 | 不支持 | 支持（Tavily + DuckDuckGo 双引擎分级降级） | 质的提升 |
...
| WebSearch（新增） | 0 | — | DefaultWebSearchService |
| Eval（新增） | 0 | — | EvalController, RagJudgeClient |
| EvalDashboard（新增） | 0 | — | EvalStatsService |
...
**缺陷预测与风险评估**：完成缺陷根因分析（字面-语义鸿沟、数据不足、向量无效检索、新增模块测试缺失），建立开发→部署→运行→维护全生命周期风险矩阵（20+项风险，含应对措施和残余风险评估），按P0-P3分级提出改进建议。

**联网搜索与评测系统验证**：通过端到端功能测试验证 Tavily/DuckDuckGo 双引擎分级降级链路和触发条件的正确性，完成 LLM-as-Judge 三维度评测体系（忠实度/答案相关性/正确性）的集成测试，包括评测事件投递/消费链路、Judge 客户端容错、采样机制和看板数据接口验证。
...
| P2 | 补充 `DefaultWebSearchService` 集成测试（Mock Tavily/DuckDuckGo API） | 中 | 中 | 1月内 |
| P2 | 补充 `RagJudgeClient` JSON 解析容错单元测试 | 低 | 中 | 1月内 |
| P3 | 评测 Prompt 模板 A/B 调优（对比不同评分粒度下的 Judge 一致性） | 低 | 中 | 2月内 |
...
6. **Judge 一致性测试**：同一评测输入多次调用 Judge 模型（temperature=0.0），验证评分一致性（标准差应 < 0.1）
7. **联网搜索降级场景测试**：模拟 Tavily 超时/不可用/DuckDuckGo 不可用等组合场景，验证分级降级链路的完整性
8. **评测看板数据准确性测试**：对比手动计算的分数与 `EvalStatsService` 返回的总览/趋势数据，验证聚合逻辑正确性
...
| 联网搜索功能测试报告 | `docs_my/联网检索_测试报告.md` | — |
| 评测 Prompt 模板 | `resources/prompt/eval-faithfulness.st` | 34 |
| 评测 Prompt 模板 | `resources/prompt/eval-answer-relevancy.st` | — |
| 评测 Prompt 模板 | `resources/prompt/eval-correctness.st` | — |
