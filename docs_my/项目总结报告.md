...
| 联网搜索增强 | 3 个（WebSearchService、DefaultWebSearchService、WebSearchProperties） | 1 个 | 知识库覆盖不足时自动从互联网获取信息兜底 |
| 评估系统 | 12+ 个（三维度评测器、Judge客户端、Prompt模板、MQ消费、管理后台看板） | 3 个 | 建立 LLM-as-Judge 自动化评测体系，可量化持续监控 |
...
当用户问题超出知识库覆盖范围时，即使混合检索也无法返回有效内容。系统新增联网搜索增强机制，在意图置信度不足时自动从互联网获取信息补充回答。

**双搜索引擎策略**：

`DefaultWebSearchService` 实现了 Tavily + DuckDuckGo 分级降级链路：

```java
// Tiered degradation: Tavily → DuckDuckGo → fallback to RAG
if ("tavily".equalsIgnoreCase(provider)) {
    if (apiKey != null && !apiKey.isBlank()) {
        return searchTavily(query, maxResults, apiKey);
    }
    return searchDuckDuckGo(query, maxResults);  // auto-degrade
}
```

| 引擎 | 类型 | 需要 Key | 特点 |
|------|------|---------|------|
| Tavily | AI 优化搜索 API | 是 | 返回 AI 总结 + 结构化结果，`search_depth=basic` |
| DuckDuckGo | 免费即时答案 API | 否 | 无需注册，作为免费兜底 |

**集成触发点**位于 `StreamChatPipeline`：

![联网检索](C:\code\ragent\ragent\docs_my\rag_websearch_fallback.png)

**Tavily 响应解析**：`include_answer=true` 使 Tavily 返回一段 AI 总结性回答，系统将其作为首个结果（标记为"AI 总结"），后续跟随详细搜索条目（title + url + content）。回答末尾自动添加"以上部分信息来源于联网搜索，仅供参考"提示。

**配置管理**（`rag.websearch` 前缀）：
...
建立基于"LLM-as-Judge"范式的自动化评测体系，覆盖 RAG 答案质量的三个核心维度。

**评测指标体系**：

| 指标 | 评测内容 | 依赖项 |
|------|---------|--------|
| 忠实度（Faithfulness） | 答案中每个关键结论是否都能从检索上下文找到依据 | 问题 + 答案 + 检索上下文 |
| 答案相关性（Answer Relevancy） | 答案是否直接回应了用户问题 | 问题 + 答案 |
| 正确性（Correctness） | 答案的事实是否正确 | 问题 + 答案 + 检索上下文 + 通用知识 |

评分采用 0-1 连续量表：PASS ≥ 0.7，WARN ≥ 0.4，FAIL < 0.4。

**架构设计**：

评测系统采用"异步事件驱动 + 策略模式 + RocketMQ"三层架构：

![评测架构](C:\code\ragent\ragent\docs_my\rag_eval_flow.png)

**关键设计点**：

- **策略模式**：`RagMetricEvaluator`（@FunctionalInterface）定义评测接口，三个实现类可独立开关（通过 `RagEvalProperties.metrics.*` 配置）
- **模板方法**：`RagJudgeClient` 封装 Prompt 渲染 → LLM 调用 → JSON 解析的通路，评测器仅提供模板路径和 slots
- **Prompt 外置**：评测模板（`.st` 格式）位于 `resources/prompt/`，与代码分离，允许无代码修改调整评测标准
- **采样机制**：`sampleRate=0.05` 控制评测采样率，避免全量评测对 API 成本和延迟的压力
- **暂存模式**：`EvalPendingStore`（ConcurrentHashMap）在 SSE 流开始前暂存问题/上下文，流完成后由 EventHandler 读取、组装评测事件

**管理后台看板**（`EvalStatsService`）：

| 接口 | 功能 | 数据维度 |
|------|------|---------|
| `loadOverview` | 总览 | 三维度平均分、低分数量与占比 |
| `loadTrends` | 趋势 | 按小时/按天的分数变化 |
| `loadLowScoreSamples` | Badcase 分析 | FAIL 样本的问题、答案、评价理由 |

评测系统与检索优化形成"评测→分析→改进→再评测"的持续改进闭环。
...
| 层次 | 覆盖范围 | 实施情况 |
|------|---------|---------|
| 单元测试 | 单个类/方法的逻辑正确性 | RRF/WeightedSum 融合策略 18 个用例，全部通过 |
...
基于评测看板和实际运行日志验证 RAG 效果评测系统的端到端正确性：

**评测触发验证**（以"PyTorch搭建CNN步骤"为例）：
...
评测系统在 SSE 流完成后自动触发，经 RocketMQ 异步消费，三个评测器（faithfulness / answer_relevancy / correctness）各返回一个评测结果。评测结果落库 `t_rag_eval_result`，同时回写 Trace 的 extraData 字段，在前端 Trace 详情页可直观查看评测分数和标签。

**联网搜索集成测试**（基于 `docs_my/联网检索_测试报告.md`）：

| 测试场景 | 知识库覆盖 | 触发联网搜索 | 结果 |
|---------|-----------|------------|------|
| "Redis介绍" | 无（关键词/向量均 0 chunk） | 是 | Tavily 返回 AI 总结 + 结构化结果，完整回答 |
| "介绍YOLO" | 无（改写后"YOLO介绍"两路均 0 chunk） | 是 | 返回 YOLO 算法原理、版本演进、优劣势，标注来源 URL |
| "PyTorch搭建CNN步骤" | 有（kbpytorch，意图命中） | 否 | 正常 RAG 链路完成，未触发联网搜索 |

联网搜索在知识库完全未覆盖的场景下（Redis、YOLO）成功提供了高质量的外部信息补充，且在知识库已覆盖的场景下（PyTorch）不会误触发，验证了触发条件的准确性。
...
| F-WEB-01 | 联网搜索接口（Tavily + DuckDuckGo 双引擎） | **新增** |
| F-WEB-02 | 搜索分级降级（Tavily → DuckDuckGo → RAG回退） | **新增** |
| F-EVAL-01~03 | 评测模式 + 文档溯源 + 评测 API | **新增** |
| F-EVAL-04~06 | 三维度评测（忠实度/答案相关性/正确性） | **新增** |
| F-EVAL-07~09 | 评测看板（总览/趋势/低分样本） | **新增** |
